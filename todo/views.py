        ...
        if request.POST["password1"] == request.POST["password2"]:
            try:
                ...
                return render(
                    ...
                    {"form": UserCreationForm, "error": "username not found"},
                )
            ...
            return render(
                request,
                "todo/signupuser.html",
                {"form": UserCreationForm, "error": "password not same"},
            )
    ...
    if request.method == "GET":
        ...
        if user is None:
            return render(
                request,
                "todo/loginuser.html",
                {"form": AuthenticationForm, "error": "username not matching"},
            )
        else:
            login(request, user)
            return redirect("currenttodos")
...
def createtodo(request):
    if request.method == "GET":
        return render(request, "todo/createtodo.html", {"form": todo_task})
    else:
        try:
            ...
            return render(
                ...
                {"form": todo_task, "error": "value exceed"},
            )
...
def viewtodo(request, todo_pk):
    ...
    if request.method == "GET":
        ...
        return render(request, "todo/viewtodo.html", {"todo": todos, "form": todoform})
    else:
        try:
            ...
            return render(
                request,
                "todo/viewtodo.html",
                {"todos": todos, "form": todoform, "error": "value error"},
            )
